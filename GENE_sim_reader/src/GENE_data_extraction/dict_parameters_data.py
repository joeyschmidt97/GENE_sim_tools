...
def create_species_tuple(parameters_filepath: str, input_spec_list:list='all'):
    ...
    if isinstance(parameters_filepath, str):
        param_dict = parameters_filepath_to_dict(parameters_filepath)
    elif isinstance(parameters_filepath, dict):
        param_dict = parameters_filepath
    else:
        raise TypeError('Please ensure input "parameters_filepath" is either a filepath or dict type.')
